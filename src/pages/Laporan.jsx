...
function Laporan() {
    ...
    return (
        <section className="container-fluid h-100 d-flex flex-column p-0" style={{ minHeight: 0 }}>
            <div className="card shadow-sm border-0 rounded-4 overflow-hidden d-flex flex-column flex-grow-1" style={{ minHeight: 0 }}>
                {/* Filter Header */}
                <div className="bg-white p-3 border-bottom flex-shrink-0">
                ...
                {result && (
                    <div className="bg-light px-3 py-2 border-bottom d-flex align-items-center justify-content-between flex-wrap gap-2 small flex-shrink-0">
                ...
                {/* Sub Navigation Tabs */}
                <div className="bg-light p-3 border-bottom flex-shrink-0">
                ...
                {error && <div className="alert alert-danger py-2 small m-3 mb-0 flex-shrink-0">{error}</div>}

                {/* Tab Content Area */}
                <div className="p-3 p-md-4 bg-white flex-grow-1 d-flex flex-column" style={{ minHeight: 0, overflow: "hidden" }}>
                    ...
                    {!loading && activeTab === "user" && (
                        <div className="d-flex flex-column flex-grow-1 h-100" style={{ minHeight: 0 }}>
                            <h6 className="fw-bold mb-3 flex-shrink-0">LAPORAN USER</h6>
                            <div className="table-responsive flex-grow-1 border rounded-3" style={{ minHeight: 0, overflowY: "auto" }}>
                                <table className="table table-bordered align-middle text-center small mb-0">
                                    ...
                                    <tbody>
                                        {dataUser.length === 0 ? <tr><td colSpan={6} className="text-muted py-3">Tidak ada data</td></tr> : dataUser.map((u, i) => (
                    ...
                    {!loading && activeTab === "masuk" && (
                        <div className="d-flex flex-column flex-grow-1 h-100" style={{ minHeight: 0 }}>
                            <h6 className="fw-bold mb-3 flex-shrink-0">LAPORAN BARANG MASUK</h6>
                            <div className="table-responsive flex-grow-1 border rounded-3" style={{ minHeight: 0, overflowY: "auto" }}>
                                <table className="table table-bordered align-middle text-center small mb-0">
                                    <thead className="table-light sticky-top" style={{ zIndex: 1 }}><tr><th>No</th><th>Tanggal</th><th>Nama Produk</th><th>Jumlah Masuk</th><th>Keterangan</th></tr></thead>
                                    <tbody>
                                        {dataMasuk.length === 0 ? <tr><td colSpan={5} className="text-muted py-3">Tidak ada data</td></tr> : dataMasuk.map((r, i) => (
                    ...
                    {!loading && activeTab === "keluar" && (
                        <div className="d-flex flex-column flex-grow-1 h-100" style={{ minHeight: 0 }}>
                            <h6 className="fw-bold mb-3 flex-shrink-0">LAPORAN BARANG KELUAR</h6>
                            <div className="table-responsive flex-grow-1 border rounded-3" style={{ minHeight: 0, overflowY: "auto" }}>
                                <table className="table table-bordered align-middle text-center small mb-0">
                                    <thead className="table-light sticky-top" style={{ zIndex: 1 }}><tr><th>No</th><th>Tanggal</th><th>Nama Produk</th><th>Jumlah Keluar</th><th>Total Harga</th></tr></thead>
                                    <tbody>
                                        {dataKeluar.length === 0 ? <tr><td colSpan={5} className="text-muted py-3">Tidak ada data</td></tr> : dataKeluar.map((r, i) => (
                    ...
                    {activeTab === "akhir" && (
                        <div className="d-flex flex-column flex-grow-1 h-100" style={{ minHeight: 0 }}>
                            <h6 className="fw-bold mb-3 flex-shrink-0">LAPORAN HASIL AKHIR</h6>
                            ...
                            <div className="table-responsive flex-grow-1 border rounded-3" style={{ minHeight: 0, overflowY: "auto" }}>
                                <table className="table table-bordered align-middle text-center small mb-0">
                                    <thead className="table-light sticky-top" style={{ zIndex: 1 }}><tr><th>No</th><th>Tanggal</th><th>Kasir</th><th>Total</th><th>Item</th></tr></thead>
                                    <tbody>
                                        {!dataAkhir || dataAkhir.transaksiList.length === 0 ? <tr><td colSpan={5} className="text-muted py-3">Tidak ada transaksi</td></tr> : dataAkhir.transaksiList.map((t, i) => (
